...
train_dataset = (
    train_dataset.map(
        lambda x, y: utils.encode_single_sample(x, y, img_height, img_width, char_to_num), num_parallel_calls=tf.data.experimental.AUTOTUNE
    )
    .batch(batch_size)
...
# Get the model
model = utils.build_model(img_width, img_height, characters)
model.summary()
...
for batch in validation_dataset.take(1):
    ...
    pred_texts = utils.decode_batch_predictions(preds, max_length, num_to_char)
